...
void IRAM_ATTR onTimer() {
  for (int i = 1; i <= availableFloppy; i++) {
    if (enableVibrato[i]) {
      if (vibratoState[i]) {
        bendFactor[i] = originalBend[i] + 0.015;
      }
      else {
        bendFactor[i] = originalBend[i] - 0.015;
      }
...
void loop() {
  ...
  for (int i = 1; i <= availableFloppy; i++) {
    if (currentPeriod[i] > 0 && currentMicros - previousMicros[i] >= currentPeriod[i]) {
      ...
      digitalWrite(stepPin[i], HIGH);
      digitalWrite(stepPin[i], LOW);
      digitalWrite(dirPin[i], floppyDir[i]);
    }
  }
}
...
void handleNoteOn(byte channel, byte note, byte velocity) {
  if (velocity == 0) {
    ...
    digitalWrite(enPin[channel], LOW);
  }
}
...
void handleNoteOff(byte channel, byte note, byte velocity) {
  if (note == currentNote[channel]) {
    ...
    digitalWrite(enPin[channel], HIGH);
  }
}
